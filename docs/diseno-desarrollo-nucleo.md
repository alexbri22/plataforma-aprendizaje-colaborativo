...
El manejador de base de datos sigue siendo una decisión abierta del equipo (11 del general) y su cierre no debe bloquear la escritura de los servicios. La capa de acceso se define por interfaz: cada módulo declara, en términos del dominio, las lecturas y escrituras que necesita, y una implementación concreta las resuelve contra el manejador que se elija. Los servicios y sus pruebas se escriben contra la interfaz desde el primer día, y la implementación se sustituye sin tocarlos.

Dos requisitos que la elección debe satisfacer, ambos derivados de decisiones ya tomadas y no de preferencias: transacciones con la granularidad que 2.4 necesita, e índice sobre la pareja de actividad y usuario en la relación de membresías, que el middleware lee en prácticamente toda petición (4.2 del general).

**Lo que no se adopta.** Un mapeador objeto-relacional que genere el esquema a partir de las clases del código. El modelo relacional está fijado en 4.4 del general y es material de la tesina; invertir la dirección haría que el esquema documentado dependiera de una traducción automática, y que cualquier cambio en el código produjera una migración no deliberada. La dirección es la contraria: el esquema es la fuente y el código lo consume.
...
Casi todas las pruebas anteriores necesitan una actividad en una fase concreta, con participantes y equipos. Construir ese estado recorriendo la aplicación en cada prueba la haría lenta y frágil, y acoplaría la prueba de un módulo a los defectos de otro. Se escribe por tanto un generador que produce una actividad en cualquier fase, con el número de participantes y equipos que se le indique, escribiendo directamente sobre el almacén. Es la primera pieza del incremento de desbloqueo de 11.2.
